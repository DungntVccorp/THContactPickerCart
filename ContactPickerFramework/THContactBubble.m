...
- (void)setupView {
    ...
    [self.btnRemove setImage:[UIImage imageNamed:@"ic_create_new_chat_delete_selected"] forState:UIControlStateNormal];
    [self.btnRemove addTarget:self action:@selector(onRemoveBubble:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)adjustSize {
    ...
    frame.origin.y = kVerticalPadding;
     UIImage *img = [self.btnRemove imageForState:UIControlStateNormal];
    
    frame.size.width = frame.size.width + frame.size.height + 2 * kVerticalPadding - 12;
    ...
    self.btnRemove.frame = CGRectMake(self.bounds.size.width - self.bounds.size.height, (self.bounds.size.height - img.size.height) / 2, self.bounds.size.height, img.size.height);
...
- (void) onRemoveBubble:(UIButton *)sender{
